classdef VideoAnnotationApp < handle
    ...
    methods (Access = public)
        ...
        function obj =  VideoAnnotationApp()
            ...
            obj.currentFile = 1;
            obj.state = VideoAnnotatorState.kAddPeaksMode;
    ...
    methods (Access = private)
        ...
        function loadUI(obj)
            obj.uiHandles = guihandles(VideoAnnotationUI);
        ...
        function selectSampleAtLocation(obj,x)
            if isempty(obj.rangeSelection)
                obj.rangeSelection = SampleRange(x);
            else
        ...
        function updateSignalsComputerList(obj)
        ...
        function outputTxt = handleUserClick(obj,src,~)
            ...
            if obj.state == VideoAnnotatorState.kAddPeaksMode
                obj.addPeakAtLocation(x);
            elseif obj.state == VideoAnnotatorState.kSelectSamplesMode
        ...
        function handleStateChanged(obj,~,~)
            ...
            if obj.state == VideoAnnotatorState.kSelectSamplesMode
                obj.deleteRangeSelection();
            end
            ...
            switch obj.uiHandles.stateButtonGroup.SelectedObject
                ...
                case (obj.uiHandles.modifyPeaksRadio)
                    obj.state = VideoAnnotatorState.kModifyPeaksMode;
                    cursorModeHandle.Enable = 'off';
                case (obj.uiHandles.deletePeaksRadio)
                    obj.state = VideoAnnotatorState.kDeletePeaksMode;
                    cursorModeHandle.Enable = 'off';
                case (obj.uiHandles.selectRangeRadio)
                    obj.state = VideoAnnotatorState.kSelectSamplesMode;
            end
        end
